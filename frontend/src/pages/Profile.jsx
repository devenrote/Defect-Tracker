  ...
  return (
    <Layout title="User Profile">
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="text-center sm:text-left space-y-1">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                <h3 className="text-base font-black text-slate-800 dark:text-white leading-tight">{user?.full_name || 'System User'}</h3>
        ...
        {/* Edit Profile Form */}
        <form onSubmit={handleSubmit(onSubmit)} className="card space-y-4 shadow-xs">
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 tracking-wide">New Password (optional)</label>
            <input 
              type="password" 
          ...
          <div className="flex justify-end pt-2">
            <button type="submit" disabled={loading} className="btn-primary w-full sm:w-auto font-semibold text-xs flex items-center justify-center gap-1.5 py-2 cursor-pointer shadow-xs">
              <Save className="w-3.5 h-3.5" /> {loading ? 'Saving Changes...' : 'Save Changes'}
            </button>
          </div>
        </form>
